        ...
        <div class="form-group mb-3">
            ...
            <div class="input-group">
                ...
                <select type="select" name="physical_status_id" id="physical_status_id"
                    ...
                    @isset($record['physical_statuses'])
                        @foreach ($record['physical_statuses'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('physical_status_id') ?? ($profile->physical_status_id ?? '') == $value)>{{ $label }}</option>
                        @endforeach
                    @endisset
                </select>
            </div>
    ...
    <div class="col-sm-3">
        <div class="form-group mb-3">
            <label class="form-label" for="height_id">{{ trans('fields.height') }}<span
                    class="require-star">*</span></label>
            <div class="input-group">
                <div class="input-group-prepend"><span class="input-group-text"><i class="fas fa-hashtag"></i></span>
                </div>
                <select type="select" name="height_id" id="height_id" class="form-control aiz-selectpicker required "
                    data-live-search="true" -data-width="auto">
                    <option style="display:none" value="">-- Select --</option>
                    @isset($record['heights'])
                        @foreach ($record['heights'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('height_id') ?? ($profile->height_id ?? '') == $value)>{{ $label }}</option>
                        @endforeach
                    @endisset
                </select>
            </div>
    ...
    <div class="col-sm-3">
        <div class="form-group mb-3">
            ...
            <div class="input-group">
                ...
                <select type="select" name="weight_id" id="weight_id" class="form-control aiz-selectpicker required "
                    data-live-search="true" -data-width="auto">
                    <option style="display:none" value="">-- Select --</option>
                    @isset($record['weights'])
                        @foreach ($record['weights'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('weight_id') ?? ($profile->weight_id ?? '') == $value)>{{ $label }}</option>
                        @endforeach
                    @endisset
                </select>
            </div>
                ...
                <select type="select" name="body_type_id" id="body_type_id"
                    ...
                    @isset($record['body_types'])
                        @foreach ($record['body_types'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('body_type_id') ?? ($profile->body_type_id ?? '') == $value)>{{ $label }}</option>
                        @endforeach
                    @endisset
                </select>
            </div>
    ...
    <div class="col-sm-3">
        <div class="form-group mb-3">
            ...
            <div class="input-group">
                ...
                <select type="select" name="color_id" id="color_id" class="form-control aiz-selectpicker required "
                    data-live-search="true" -data-width="auto">
                    <option style="display:none" value="">-- Select --</option>
                    @isset($record['colors'])
                        @foreach ($record['colors'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('color_id') ?? ($profile->color_id ?? '') == $value)>{{ $label }}</option>
                        @endforeach
                    @endisset
                </select>
            </div>
                ...
                <select type="select" name="blood_group_id" id="blood_group_id"
                    ...
                    @isset($record['blood_groups'])
                        @foreach ($record['blood_groups'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('blood_group_id') ?? ($profile->blood_group_id ?? '') == $value)>{{ $label }}</option>
                        @endforeach
                    @endisset
                </select>
            </div>
